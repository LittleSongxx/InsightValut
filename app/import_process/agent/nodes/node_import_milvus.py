...
def create_collection(client, collection_name: str, vector_dimension: int):
    ...
    )  # 图片URL列表（JSON数组格式，用于查询时直接获取图片而非正则提取）
    schema.add_field(
...
def step_2_prepare_collection(vector_dimension: int):
    ...
    # 强制重建标记：设为 True 时，若集合已存在则先删除再重建（用于 schema 变更后同步）
    force_recreate = os.getenv("MILVUS_FORCE_RECREATE_COLLECTION", "0") in ("1", "true", "True")

    # 3. 集合不存在则自动创建；或强制重建标记打开时删除重建
    if not client.has_collection(collection_name=CHUNKS_COLLECTION_NAME):
        ...
        logger.warning(f"Milvus集合{CHUNKS_COLLECTION_NAME}已存在，强制重建...")
        client.drop_collection(collection_name=CHUNKS_COLLECTION_NAME)
        create_collection(client, CHUNKS_COLLECTION_NAME, vector_dimension)
        logger.warning(f"Milvus集合{CHUNKS_COLLECTION_NAME}已强制重建完成")
    else:
        logger.info(f"Milvus集合{CHUNKS_COLLECTION_NAME}已存在，直接复用（注意：schema变更不会生效，需设置MILVUS_FORCE_RECREATE_COLLECTION=1强制重建）")
