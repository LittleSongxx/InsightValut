...
def node_bge_embedding(state: ImportGraphState) -> ImportGraphState:
    ...
    try:
        ...
        logger.info(
            f"--- BGE-M3 向量化处理完成，共处理 {len(output_data)} 条文本切片 ---"
        )
...
def step_2_init_model():
    ...
    try:
        ...
        if ef is None:
            raise ValueError(
                "BGE-M3模型实例为None：pymilvus.model模块未找到或模型加载失败"
            )
    ...
    for i in range(0, total, batch_size):
        ...
        try:
            ...
            for doc in batch_texts:
                fallback_item_name = (doc.get("file_title") or "unknown_item").strip()
                item_name = (doc.get("item_name") or fallback_item_name).strip()
            ...
            if not docs_embeddings:
                logger.warning(
                    f"第{start_idx}-{end_idx}条切片：向量生成返回空，保留原数据"
                )
            ...
            for j, doc in enumerate(batch_texts):
                item = doc.copy()
                fallback_item_name = (item.get("file_title") or "unknown_item").strip()
                item["item_name"] = (
            ...
            # 批次失败直接抛错，避免下游拿到缺失向量字段的数据
            logger.error(
                f"第{start_idx}-{end_idx}条切片：向量生成失败 | 错误原因：{str(e)}",
                exc_info=True,
            )
            raise RuntimeError(f"第{start_idx}-{end_idx}条切片向量化失败") from e

    # 向下游保证结构完整，防止入库节点出现字段缺失
    missing_vector_count = sum(
        1
        for item in output_data
        if "dense_vector" not in item or "sparse_vector" not in item
    )
    if missing_vector_count:
        raise ValueError(f"向量化结果缺失字段，异常切片数：{missing_vector_count}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(
            f"=== 向量化节点本地测试失败 ===" f"错误原因：{str(e)}", exc_info=True
        )
        # 新手友好提示：给出核心排查方向
        logger.warning(
            "排查提示：请检查BGE-M3模型路径、显存是否充足、环境变量配置是否正确"
        )
