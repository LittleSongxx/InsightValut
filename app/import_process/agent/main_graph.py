...
from app.import_process.agent.nodes.node_entry import (
    node_entry,
)  # 入口节点：初始化参数、校验输入
from app.import_process.agent.nodes.node_pdf_to_md import (
    node_pdf_to_md,
)  # PDF转MD：解析PDF文件为markdown格式
...
from app.import_process.agent.nodes.node_import_kg import (
    node_import_kg,
)  # 导入Neo4j：将切片数据写入知识图谱
...
)  # 向量入库：将向量数据持久化到Milvus
workflow.add_node(
    "node_import_kg", node_import_kg
)  # 图谱入库：将切片关系写入Neo4j知识图谱
...
workflow.add_edge(
    "node_import_milvus", "node_import_kg"
)  # Milvus入库完成 → 知识图谱入库
workflow.add_edge(
    "node_import_kg", END
)  # 知识图谱入库完成 → 工作流执行结束（END是内置结束节点）
